...
---
This article is based on a demo I gave in one of the Theoretical Systems Biology Tech Talk Sessions on 4th of February.
...
Baseline
-----------
Before starting any optimisations, let's first verify that the code works at all:
...
The output seems reasonable, which is great news for us.
Now let's check how fast the code runs. Unix command `time` is perfect for  this task. 

{% highlight bash %}
> time python alignment.py
{% endhighlight %}

We get the baseline of around 0m0.502s.

At this point it is curious to test it on an optimising python interpreter, [PyPy](http://pypy.org).
Download and install PyPy according to instructions in [their download page](http://pypy.org/download.html ).
If you are using OSX, Homebrew[http://brew.sh/] could be used to install PyPy:

{% highlight bash %}
> brew install pypy
{% endhighlight %}

Now we know that PyPy is an optimising Python compiler, therefore would expect the code to run faster than on CPython. To check this, we can again use `time`:
...
Now let's see how we can speed things up even further.
...
Using cProfile is as simple as typing:
...
------------------------------------------------------
From the profile above we can see that the majority of the time is spent in the `align` function, just as wee xpected, 35% of which in function `max`. 
...
Maybe using `max` function for three elements only is an overkill. Maybe we were too smart here and could actually get away with simpler code? Lets replace the max line with the following block where we do just nested ifs:
...
Running cProfile again, we can see that align function is still the bottleneck. Unfortunately, it does not tell us why any more:
...
---------------------------
Let's see if we can speed up these lines with the help of [Cython](http://cython.org/).
Cython can be thought to be a compiler for Python that is able to convert the Python code into C.
In order to start using it, we need to install it first:
...
Line eight in this file says "compile file `align.pyx` into a library `align_cythonised`".

Now let's quickly compile our code using this setup file (do not forget the `--inplace` tag, as otherwise it would compile it into `build/` directory):

{% highlight bash %}
> python setup.py build_ext --inplace
{% endhighlight %}

This generates `align_c` and `align_cythonised.so` files.

We could now use [`timeit`](http://docs.python.org/2/library/timeit.html) module to compare the runtimes.
...
Note that PyPy runs this function in 168 ms. One could be naughty and overstretch this result and say that Python runs faster than C (hehe).
...
To do so, let's change our matrix to be a numpy n-dimensional array rather than a dictionary.
...
You might need a Fortran compiler to do so (you can obtain it from Homebrew on OSX `brew install gfortran`).

Once this is all set, 
Copy `align.pyx` into `align_numpy.pyx` and change it slightly to use `numpy`.
...
As you can see, the total runtime is 101 milliseconds. This is 157ms faster than the original version of the code and all we did was to start using numpy and add a couple of type hints.  This Cythonic code can be further optimised by disabling things like bound checking and other dangerous things.

Furthermore, Cython provides one of the most convenient ways of running [native] C code from python.
...
In conclusion, we were able to significantly improve runtimes of Python code by profiling where the bottlenecks are and doing simple, yet incremental changes to it. In essence, alternative interpreters/compilers for python should be considered only as a last-resort. More often than not, there is a way to obtain sufficient performance boosts from simply changing the bits of the code, as we did in the beginning.  But whether this would prove insufficient for your needs, these optimisation tools exist already and will continue to improve over time.
